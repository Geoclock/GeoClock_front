...
class DefaultScreen extends Component {
    ...
    GetNote = (geo_id) => {
        ...
        this.notesubjection.map((notesub) => {
            if(geo_id === notesub.geo_id){
                this.notifications.map((note) => {
                    if(notesub.note_id === note.id){
                        g= note;
                    }
                });
            }
            ...
            <View
                ...
                <MapView
                    ...
                    {this.state.markers.map((marker,i)=>{
                        return (
                            <MapView.Marker key={i} coordinate={marker.coordinate} onPress={()=>{this.state.showWindow.index=i,this.state.showWindow.show=true}}><Image style = {{ width: 39, height: 55,}} source = {require('../assets/marker.png')}/></MapView.Marker>
                        )
                    })}
                </MapView>
...
export default withNavigation(connect(mapStateToProps)(DefaultScreen))
